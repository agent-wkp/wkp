...
**M6 started (issue #172, `.github/workflows/release.yml`).** A release workflow now exists: it builds `wkp` for every cross-compile target twice, on two independent runners, with `SOURCE_DATE_EPOCH` and `--locked`, and fails if the two attempts' hashes don't match -- the reproducible-build piece of this section. **Not yet verified against a real release**: pushing a test tag to exercise it live was blocked (a visible action to the shared repo); it merged on code review alone, per direction to fix forward against the first real `v*` tag if something's wrong rather than a synthetic test tag. Signing (cosign+SLSA, issue #173), SBOM (issue #174), and actual publication (today it only creates a draft release, explicitly unsigned) remain not built. The release profile and size/startup budget below are the other part of this section already true, since they're part of `Cargo.toml` itself, not the release pipeline.
