...
|---|---|---|---|---|
| raw | 1 min | 240 | 6 hours | "why did this session feel slow" |
| 15-minute | 15 min | 1,344 | 14 days | recent-history trend |
| 6-hour | 6 hours | 1,460 | 1 year | long-term trend line only |

Each step divides the next evenly (1 → 15 → 360 minutes) so bucket boundaries align. At roughly 20 metrics (the fixed set of `wkp` subcommands plus a couple of gauges) × 3,044 slots/metric, `.wkp/metrics.db` stabilizes around 3.5-4 MB -- sized for "a session's worth of detail, a couple of weeks of trend, a year of coarse history," not for a continuously-ticking infrastructure counter; RRDtool-style defaults (e.g. a year at hourly resolution) would be roughly 5-8x larger for no benefit a bursty, human/agent-paced CLI would ever query for.
...
- **macOS (documented limitation, ADR-0015):** no native write-restriction mechanism is built. Hardened-runtime entitlements govern JIT/library-validation/notarization, not filesystem access, so they don't fit this need; Seatbelt's self-apply API (`sandbox_init`) is private, undocumented, deprecated with no replacement, and unsafe to wrap given `wkp-cli`'s `#![forbid(unsafe_code)]`; re-exec'ing under the external `sandbox-exec` binary is the self-invocation bug class CLAUDE.md documents an incident for. Running `wkp` natively on macOS today gets the OS's ordinary per-app protections only, not this section's Linux guarantee. Issue #225 tracks an opt-in `--sandbox podman` flag (re-exec inside a container, no recursion risk since the container's own entrypoint never re-passes `--sandbox`) as the actual path to filesystem isolation on macOS, not a default-on mechanism.
...
3. **~~macOS sandboxing.~~ Resolved (ADR-0015, 2026-09-16):** no native mechanism will be built (Seatbelt's self-apply API is private/deprecated/unsafe-to-wrap-safely; a `sandbox-exec` re-exec is the self-invocation bug class CLAUDE.md already documents an incident for). Documented as a known limitation instead; issue #225 tracks an opt-in `--sandbox podman` flag as the actual isolation path for macOS users who want it.
