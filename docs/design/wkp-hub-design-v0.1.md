...
**M6 in progress (issues #172/#173/#174/#176, `.github/workflows/release.yml`).** A release workflow now exists, covering every clause of this section's own bullet list below except distribution channels beyond the OCI image and self-update verification (issues #175/#177, separate tasks): builds `wkp` for every cross-compile target twice, on two independent runners, with `SOURCE_DATE_EPOCH` and `--locked`, failing if the two attempts' hashes don't match; signs every binary keylessly with cosign (Fulcio + Rekor, no private key this repo has to protect), self-verified with `cosign verify-blob` in the same job; generates SLSA Build L3 provenance via the official generic generator over the exact binaries shipped; generates an SBOM (CycloneDX and SPDX, via Syft against `Cargo.lock`); builds and pushes a signed multi-arch (`linux/amd64`/`linux/arm64`) OCI image to `ghcr.io/<owner>/wkp`. Publishes all of it as a **draft** GitHub Release -- not published for real yet, since Homebrew tap/self-update verification/sandboxing (issues #175, #177, #170-#171) are still open, and a human decides when a draft here actually becomes real. **Not yet verified against a real release**: pushing a test tag to exercise any of this live was blocked (a visible action to the shared repo); merged on code review alone each time, per direction to fix forward against the first real `v*` tag if something's wrong rather than a synthetic test tag. The release profile and size/startup budget below are the other part of this section already true, since they're part of `Cargo.toml` itself, not the release pipeline.
